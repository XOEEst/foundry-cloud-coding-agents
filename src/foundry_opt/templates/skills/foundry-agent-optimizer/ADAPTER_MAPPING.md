...
- **Candidate designer** — receives only a canonical `CandidateDesignIntent` from the steward and
  ...
  specialist that brainstorms or writes candidate code. The steward, not the designer, packages
  Foundry drafts, runs evaluations, computes eligibility, and records the durable scoreboard.
